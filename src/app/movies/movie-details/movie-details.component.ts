...
export class MovieDetailsComponent implements OnInit {
  ...
  ngOnInit(): void{
      this.activatedRoute.params.subscribe(params => {
        this.moviesService.getById(params['id']).subscribe(movie => {
          ...
          this.trailerURL= this.generateYoutubeURLForEmbedeedVideo(movie.trailer)
          this.coordinates = movie.movieTheaters.map(movieTheater => {
            return {latitude: movieTheater.latitude, longtitude: movieTheater.longitude,
  ...
  generateYoutubeURLForEmbedeedVideo(url: any): SafeResourceUrl
  {
    if(!url)
    {
      return '';
